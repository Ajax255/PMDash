<template>
  <TransitionRoot as="template" :show="showTaskModal">
    <Dialog as="div" class="relative z-10" @close="$emit('closeTaskModal')">
      <TransitionChild
        as="template"
                  ...
                  <div class="space-y-6">
                    <div class="space-y-8 divide-y divide-gray-200 sm:space-y-5">
                      <div>
                        <div>
                          <h3
                            v-if="taskStore.taskMode === ModalMode.create"
                            class="text-lg leading-6 font-medium text-gray-900"
                          >
                            New Task
                          </h3>
                          <h3
                            v-if="taskStore.taskMode === ModalMode.edit"
                            class="text-lg leading-6 font-medium text-gray-900"
                          >
                            Edit Task
                          </h3>
...
const onSubmit = () => {
  if (taskStore.taskMode === ModalMode.edit) {
    updateTask();
    emit('closeTaskModal');
  } else if (taskStore.taskMode === ModalMode.create) {
    createTask();
    emit('closeTaskModal');
  } else {
    emit('closeTaskModal');
...
const createTask = async () => {
  await taskStore.createTask(taskStore.task);
  await taskStore.fetchAllTasks();
};

const updateTask = async () => {
  await taskStore.updateTask(taskStore.task._id, taskStore.task);
  await taskStore.fetchAllTasks();
};
</script>
