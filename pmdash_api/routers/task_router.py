...
@router.get("/fetch-all-tasks", response_description="List all tasks", response_model=List[Task])
...
@router.put("/update/{uuid}", response_description="Update a Task", response_model=Task)
async def update_task(uuid: str, task: UpdateTask = Body(...)):
    task = {k: v for k, v in task.dict().items() if v is not None}

    if len(task) >= 1:
        update_result = await mongodb.update_one(data_base=DATABASE, collection=TASKS_COLLECTION, query={"_id": uuid}, update={"$set": task}),
...
@router.delete("/delete/{uuid}", response_description="Delete a Task")
